(() => {
    if (typeof globalThis !== 'undefined') {
        ...
        if (!globalThis.chrome.webNavigation) {
            const dummyEvent = { addListener() {}, removeListener() {}, hasListener() { return false; } };
            globalThis.chrome.webNavigation = new Proxy({
                getAllFrames(details, cb) { if (cb) cb([]); return Promise.resolve([]); },
                getFrame(details, cb) { if (cb) cb(null); return Promise.resolve(null); }
            }, {
                get(target, prop) {
...
if (!(globalThis.browser && globalThis.browser.runtime && globalThis.browser.runtime.id)) {
    ...
    const wrapped = wrapAPIs(chrome);
// Ensure the dummy webNavigation object from the IIFE is used
wrapped.webNavigation = globalThis.chrome.webNavigation;
